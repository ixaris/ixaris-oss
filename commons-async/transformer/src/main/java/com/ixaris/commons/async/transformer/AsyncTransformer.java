...
import static com.ixaris.commons.async.transformer.AsmHelper.createHandle;
...
/**
 * This transformer transforms seemingly blocking code, specifically using Async.await() to await future results, into
 * asynchronous code using stackless continuations.
 *
 * @author brian.vella
 */
final class AsyncTransformer {
    ...
    static {
        // eclipse compiler does something, probably with access control, that prevents classes from being loaded during
        // compilation, so we preload all the classes from this package
        Preloader.preloadClassesInPackage(
            AsyncTransformer.class.getClassLoader(), "com.ixaris.commons.async.transformer"
        );
    }
    ...
    private static final Set<String> ASYNC_AWAIT_METHODS = Stream.of(
        ASYNC_AWAIT_METHOD_NAME, ASYNC_AWAIT_EXCEPTIONS_METHOD_NAME
    )
    ...
    private static final String COMP_STAGE_UTIL_DONE_COMPOSE_DESC = String.format(
        "(L%s;L%s;)L%s;", COMP_STAGE_NAME, FUNCTION_THROWS_NAME, COMP_STAGE_NAME
    );
    ...
    private static final Handle LAMBDAMETAFACTORY_HANDLE = createHandle(
        H_INVOKESTATIC,
    ...
    AsyncTransformer() {
        this((fqcn, method, line, message) -> System.err.println(fqcn + "." + method + ":" + line + " - " + message));
    }
    ...
    byte[] transform(final ClassReader cr) throws AnalyzerException {
        ...
        for (final MethodNode original : new ArrayList<>(classNode.methods)) {
            ...
            if (shouldTransform) {
                ...
                changed |=
                    !isAbstract && transformAsyncMethod(classNode, original, nameUseCount, retType.getInternalName());
            } else {
    ...
    private static class CheckSyncMethodVisitor extends MethodVisitor {
        ...
        public void visitMethodInsn(
            ...
            if (isOneOfAwaitMethods(opcode, owner, name)) {
                throw error(
                    extractFullyQualifiedClassName(classNode),
                    lastLine,
                    "non-async methods should not call await() and awaitExceptions(). To block, use block()"
                );
            } else {
    ...
    private static class TransformAsyncMethodVisitor extends MethodVisitor {
        ...
        private TransformAsyncMethodVisitor(
        ...
        public void visitMethodInsn(
            ...
            if (isOneOfAwaitMethods(opcode, owner, name)) {
                ...
                transformAwait(
                    ASYNC_AWAIT_METHOD_NAME.equals(name),
                    classNode,
                    original,
                    this,
                    awaitSwitchEntry,
    ...
    private boolean transformAsyncMethod(
        final ClassNode classNode,
        final MethodNode original,
        ...
        continuation.visitTableSwitchInsn(
            0,
            switchEntries.size(),
            defaultLabel,
            Stream
                .concat(Stream.of(entryPoint.resumeLabel), switchEntries.stream().map(e -> e.resumeLabel))
                .toArray(Label[]::new)
        );
        ...
        original.accept(
            new TransformAsyncMethodVisitor(
                continuation,
                replacement,
                classNode,
                original,
        ...
        replacement.visitMethodInsn(
            isStatic
                ? INVOKESTATIC
                : ((classNode.version <= 52) ? INVOKESPECIAL : (isInterface ? INVOKEINTERFACE : INVOKESPECIAL)),
            classNode.name,
            continuation.name,
            continuation.desc,
            isInterface
    ...
    private List<Argument> determineArgumentsAndFixObjectInitialization(
        final MethodNode original, final Frame<BasicValue>[] frames, final List<AwaitSwitchEntry> switchEntries
    ) {
        final boolean isStatic = Modifier.isStatic(original.access);
        ...
        for (final AwaitSwitchEntry se : switchEntries) {
            ...
            for (
                ...
                local < se.frame.getLocals();
                local += valueSize(se.frame.getLocal(local))
            ) {
    ...
    private List<AwaitSwitchEntry> determineSwitchEntriesForAwaitCalls(
        final ClassNode classNode, final MethodNode original, final Frame<BasicValue>[] frames
    ) {
    ...
    private Object[] determineContinuationFrame(
        final boolean isStatic, final ClassNode classNode, final List<Argument> arguments
    ) {
    ...
    private String determineContinuationLambdaDesc(
        final boolean isStatic, final ClassNode classNode, final Type[] typeArguments
    ) {
    ...
    private void mapLocalToLambdaArgument(
        final MethodNode method,
        ...
        final Argument argument = oargument.orElseGet(() ->
            addArgument(name, value, Modifier.isStatic(method.access), arguments)
        );
        argument.tmpLocalMapping = local;
    }
    
    private static Argument addArgument(
        final String name, final BasicValue value, final boolean isStatic, final List<Argument> arguments
    ) {
        final int argOffset;
    ...
    private static boolean isLocalVariableInScope(
        final MethodNode method, final AwaitSwitchEntry se, final LocalVariableNode node
    ) {
    ...
    static void replaceObjectInitialization(
        final MethodNode method, final Frame<BasicValue>[] frames, final Set<AbstractInsnNode> objectCreationNodes
    ) {
        int originalLocals = method.maxLocals;
        final Set<AbstractInsnNode> uninitializedObjects = objectCreationNodes != null
            ? objectCreationNodes
            : Stream.of(method.instructions.toArray()).filter(i -> i.getOpcode() == NEW).collect(Collectors.toSet());
        ...
        for (
            ...
            insnNode != null;
            index++, insnNode = insnNode.getNext()
        ) {
            if (insnNode instanceof FrameNode) {
                ...
                if (methodInsnNode.name.equals("<init>")) {
                    insnNode = replaceConstructorCall(
                        method, frames[index], uninitializedObjects, originalLocals, methodInsnNode
                    );
                }
            }
        }
    ...
    private static AbstractInsnNode replaceConstructorCall(
        final MethodNode method,
        ...
        for (int iLocal = originalLocals, j = frame.getStackSize(); --j >= firstOccurrence; ) {
            ...
            if (!target.equals(value)) {
                ...
                currentInsn = insertInstruction(
                    instructions, currentInsn, new VarInsnNode(value.getType().getOpcode(ISTORE), iLocal)
                );
        ...
        for (int j = firstOccurrence + repetitions; j < frame.getStackSize(); j++) {
            ...
            if (value.getType() != null) {
                instructions.insert(
                    currentInsn, currentInsn = new VarInsnNode(value.getType().getOpcode(ILOAD), stackToLocal[j])
                );
            } else {
                // uninitialized value
    ...
    private static AbstractInsnNode insertInstruction(
        final InsnList instructions, final AbstractInsnNode currentInsn, final AbstractInsnNode nextInsn
    ) {
        instructions.insert(currentInsn, nextInsn);
        return nextInsn;
    }
    
    private static List<Object> replaceUninitializedFrameValues(
        final Set<AbstractInsnNode> uninitializedObjects, final List<Object> list
    ) {
        if (list == null) {
            return null;
        }
    ...
    /**
     * Replaces calls to Await.await with getting a promise if done: keep executing otherwise, call continuation and
     * jump to the correct place when done
     *
     ...
     */
    private static void transformAwait(
        final boolean unwrapResult,
        final ClassNode classNode,
        final MethodNode original,
        ...
        // stack: { ... stage stage }
        mv.visitMethodInsn(
            INVOKESTATIC, COMP_STAGE_UTIL_NAME, COMP_STAGE_UTIL_IS_DONE_NAME, COMP_STAGE_UTIL_IS_DONE_DESC, false
        );
        ...
        mv.visitMethodInsn(
            ...
            COMP_STAGE_UTIL_DONE_COMPOSE_NAME,
            COMP_STAGE_UTIL_DONE_COMPOSE_DESC,
            false
        );
        ...
        if (awaitSwitchEntry.frame.monitors.length > 0) {
            for (int i = awaitSwitchEntry.frame.monitors.length; --i >= 0; ) {
                ...
                for (
                    int iLocal = 0;
                    iLocal < awaitSwitchEntry.frame.getLocals();
                    iLocal += valueSize(awaitSwitchEntry.frame.getLocal(iLocal))
                ) {
        ...
        // {  ... async? async }
        mv.visitMethodInsn(
            INVOKESTATIC, COMP_STAGE_UTIL_NAME, COMP_STAGE_UTIL_GET_NAME, COMP_STAGE_UTIL_GET_DESC, false
        );
    ...
    private static void pushInitial(
        final ClassNode classNode, final MethodNode method, final List<Argument> lambdaArguments
    ) {
        // stack: { ... }
    ...
    private static void pushDefault(
        final ClassNode classNode, final MethodVisitor mv, final BasicValue value, final int lineNumber
    ) {
        if (value.getType() == null) {
        ...
        switch (value.getType().getSort()) {
            ...
            default:
                throw error(
                    extractFullyQualifiedClassName(classNode), lineNumber, "Unknown type: %s", value.getType().getSort()
                );
        }
    }
    
    private static void restoreStackAndLocals(
        final ClassNode classNode,
        final MethodNode original,
        final MethodVisitor mv,
        final AwaitSwitchEntry se,
    ...
    private static void reacquireMonitors(
        final ClassNode classNode,
        final MethodNode original,
        final MethodVisitor mv,
        final AwaitSwitchEntry se,
    ...
    private void warn(
        final String fqcn, final String method, final int lineNumber, final String format, final Object... args
    ) {
        final String message = fqcn + ((lineNumber > 0) ? ":" + lineNumber + " " : " ") + String.format(format, args);
        warnConsumer.warn(fqcn, method, lineNumber, message);
    }
    
    private static AsyncTransformerException error(
        final String fqcn, final int lineNumber, final String format, final Object... args
    ) {
        final String message = fqcn + ((lineNumber > 0) ? ":" + lineNumber + " " : " ") + String.format(format, args);
