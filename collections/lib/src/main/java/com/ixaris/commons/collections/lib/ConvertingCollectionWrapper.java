...
/**
 * Wraps a collection and converts the items from one class to another. This is used to avoid iterating through the collection twice
 *
 ...
 */
public abstract class ConvertingCollectionWrapper<X, E, C extends Collection<X>> extends AbstractCollection<E> implements Collection<E>, Wrapper<C> {
    ...
    /**
     * Reverse convert from the new type to the original type. For a read only conversion, an UnsupportedOperationException can be thrown
     *
     * @param item The item
     * @return the converted item
     */
