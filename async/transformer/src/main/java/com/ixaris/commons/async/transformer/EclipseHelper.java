...
/**
 * Handler for the eclipse compiler that intercepts calls to org.eclipse.jdt.internal.compiler.ICompilerRequestor.acceptResult() and rewrites the
 * written class.
 *
 * <p>Approach was determined after looking at eclipse compiler code. This was done using mvnDebug with a dependency on
 * org.codehaus.groovy:groovy-eclipse-compiler:2.9.2-01 and org.codehaus.groovygroovy-eclipse-batch:2.4.3-01, halting during annotation
 * processing and working backwards.
 */
public class EclipseHelper implements Helper {
    ...
    static {
        ...
        Preloader.preloadClassesInPackage(AsyncTransformer.class.getClassLoader(), "org.objectweb.asm");
                    ...
                    if (currentDestinationPath != null) {
                        ...
                        for (int fileCount = classFiles.length; i < fileCount; ++i) {
                            ...
                            absolutePathsToTransform[i] =
                                currentDestinationPath + File.separatorChar + new String(filename).replace('/', File.separatorChar) + ".class";
                        }
                    }
                }
