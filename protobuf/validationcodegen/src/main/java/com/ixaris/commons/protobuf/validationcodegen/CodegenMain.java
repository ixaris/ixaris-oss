...
public final class CodegenMain implements ProtocPlugin {
    ...
    private static void extractMappings(final FileDescriptorProto fileDescriptor) {
        ...
        final String javaPackage = determineJavaInnerPackage(fileDescriptor,
    ...
    private static void extractTypeMappings(final List<DescriptorProto> descriptors, final String protoPackage, final String javaPackage) {
        descriptors.stream()
    ...
    private static void extractEnumMappings(final List<EnumDescriptorProto> descriptors, final String protoPackage, final String javaPackage) {
    ...
    private static void parseAndGenerateValidations(final FileDescriptorProto fileDescriptor) {
        ...
        fileDescriptor
            ...
            .forEach(descriptor -> {
                final String message = renderMessage(descriptor,
                    javaPackage,
                    protoPackage,
                    messagePackage,
                ...
                if (fileDescriptor.getOptions().getJavaMultipleFiles()) {
                    ...
                    FILES.add(File.newBuilder()
                        .setName((javaPackage + "." + descriptor.getName() + "Validation").replaceAll("\\.", "/") + ".java")
                        .setContent(rendered)
                        .build());
    ...
    private static String renderMessage(final DescriptorProto descriptor,
        ...
        descriptor
            .getFieldList()
            .forEach(field -> extractFieldValidationInfo(concatenatePackage(protoPackage, descriptor.getName()),
                descriptor,
                field,
                fieldValidations,
                fieldDependencies,
        ...
        descriptor
            ...
            .forEach(nestedDescriptor -> {
                messages.add(renderMessage(nestedDescriptor,
                    javaPackage,
                    protoPackage,
    ...
    private static void extractFieldValidationInfo(final String protoPackage,
        ...
        if (repeated && JavaType.MESSAGE.equals(javaType)) {
            keyValueDescriptor = descriptor
                .getNestedTypeList()
                .stream()
                .filter(n -> n.getOptions().getMapEntry() && typeName.equals("." + concatenatePackage(protoPackage, n.getName())))
                .findAny()
                .orElse(null);
        } else {
        ...
        extractFieldValidationInfoAndDependencies(protoPackage,
            field.getName(),
            repeated,
            mapField,
            javaType,
    ...
    private static void extractMessageValidationInfo(final DescriptorProto descriptor,
