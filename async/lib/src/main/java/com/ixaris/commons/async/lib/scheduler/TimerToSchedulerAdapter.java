...
final class TimerToSchedulerAdapter implements Scheduler {
    ...
    private static final class ScheduledFutureImpl<T> implements ScheduledFuture<T> {
        ...
        static ScheduledFutureImpl<Void> from(final Runnable runnable, final long msPeriod) {
            final CompletableFuture<Void> future = new CompletableFuture<>();
            final TimerTask task = (msPeriod == 0L)
                ? new TimerTask() {
    ...
    @Override
    public ScheduledFuture<Void> scheduleAtFixedRate(final Runnable runnable, final long initialDelay, final long period, final TimeUnit unit) {
    ...
    @Override
    public ScheduledFuture<Void> scheduleWithFixedDelay(final Runnable runnable, final long initialDelay, final long delay, final TimeUnit unit) {
