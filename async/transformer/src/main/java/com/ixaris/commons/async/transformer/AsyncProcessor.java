...
/**
 * Entrypoint to async code transformation, in the form of an annotation processor. Does not really do any annotation processing, instead
 * transforms every compiled class's bytecode.
 *
 * <p>Annotation processing can occur in rounds, where an annotation processor may signal that another round is required. It is therefore
 * required for the AsyncTransformer to be idempotent (which it is).
 */
...
public class AsyncProcessor extends AbstractProcessor {
    ...
    private Helper determineAndConstructHelper(final ProcessingEnvironment procEnv) {
        if (procEnv.getClass().getName().startsWith("org.eclipse")) {
            try {
                return (Helper) Class.forName("com.ixaris.commons.async.transformer.EclipseHelper").getDeclaredConstructor().newInstance();
            ...
            try {
                return (Helper) Class.forName("com.ixaris.commons.async.transformer.JavacHelper").getDeclaredConstructor().newInstance();
            } catch (final ReflectiveOperationException e) {
                throw new IllegalStateException(e);
            }
        }
