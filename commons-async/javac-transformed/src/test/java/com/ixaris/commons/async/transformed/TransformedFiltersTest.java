...
public class TransformedFiltersTest {
    ...
    public interface TestFilterManualTransformation extends AsyncFilter<String, String> {
        ...
        default CompletionStage<String> continuation$doFilter(
            final String origRequest,
            ...
            switch (async$index) {
                case 0:
                    ...
                    if (!CompletionStageUtil.isDone(async$stage)) {
                        return CompletionStageUtil.doneCompose(async$stage, f ->
                            continuation$doFilter(origRequest, next, null, (short) 1, f)
                        );
                    }
                case 1:
                    ...
                    if (!CompletionStageUtil.isDone(async$stage)) {
                        ...
                        return CompletionStageUtil.doneCompose(async$stage, f ->
                            continuation$doFilter(origRequest, next, finalRequest, (short) 2, f)
                        );
                    }
                case 2:
    ...
    public void testFilterLambda() throws InterruptedException {
        ...
        final AsyncFilterChain<String, String> filterChain = new AsyncFilterChain<>((in, next) ->
            next.next("1" + in).map(s -> s + "1")
        );
    ...
    public void testFilterDefaultMethodManuallyTransformed() throws InterruptedException {
        ...
        final AsyncFilterChain<String, String>
        filterChain = new AsyncFilterChain<>(new TestFilterManualTransformation() {
