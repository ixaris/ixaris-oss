...
public final class DoneCompletionStageAssert<T> extends ObjectAssert<CompletionStage<T>> {
    ...
    /**
     * Assert that the promise has been fulfilled (i.e. completed successfully with a result) and convert this result using the provided
     * converter function.
     */
    ...
    private String getFormattedOutcomeMessage() {
        if (timedOut) {
            ...
            return Optional.ofNullable(getFormattedThrowableMessage(throwable)).orElse(String.format("Rejected with exception [%s]%n%s",
