...
class IxarisCommonsProtobufPlugin implements Plugin<Project> {
                    ...
                    main(project.sourceSets.main) {
                        ...
                        generator {
                            ...
                            target {
                                ...
                                // e.g. com.ixaris.commons:ix-commons-logging.lib becomes
                                // com.ixaris.commons.ixaris.commons.logging.lib.jooq
                                packageName = project.group + '.' + project.name.replaceAll('-', '.').replaceAll(/^ix\./, 'ixaris.') + '.jooq'

                                // replace repeated part, e.g. com.ixaris.commons.
                                // Use compile(regex) if you want case sensitive.
                                Matcher m = Pattern.compile(/((?:[^.]+\.)+)\1/).matcher(packageName)

                                if (m.find()) {
                                    packageName = m.replaceAll('$1')
                                }
