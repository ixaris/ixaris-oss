 ...
 *   <li>a path that identifies an activity within the process; should be similar in nature to method + url in HTTP,
 *       i.e. represents the resource and action, such that the same action on 2 different items has a different path
 *   <li>a hash, or fingerprint, of the payload, used as a last resort in case a process needs to perform the same
 *       operation on the same instance with different payloads
 * </ul>
 *
 * The only scenario which is ambiguous is performing the same operation on the same path with the same payload twice
 * legitimately, however, so far we have not found a realistic use case for this (retrying is not one of them, as that
 * use case actually wants idempotency semantics)
 */
