...
public final class AsyncResult<T, E extends Throwable> {
    ...
    public static <T> List<Async<AsyncResult<T, RuntimeException>>> convert(final List<? extends CompletionStage<T>> stage) {
        return stage.stream().map(AsyncResult::convert).collect(Collectors.toList());
    }
    
    public static <K, V> Map<K, Async<AsyncResult<V, RuntimeException>>> convert(final Map<K, ? extends CompletionStage<V>> map) {
    ...
    @SuppressWarnings("squid:S1181")
    public static <T, E extends Throwable> Async<AsyncResult<T, E>> convert(final CompletionStageCallableThrows<T, E> callable) {
        try {
