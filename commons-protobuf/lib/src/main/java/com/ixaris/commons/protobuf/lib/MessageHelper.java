...
public final class MessageHelper {
    ...
    public static <T extends MessageLite> T parse(
        final T defaultInstance, final ByteString bytes
    ...
    @SuppressWarnings("unchecked")
    public static <T extends MessageLite> T parse(
        final T defaultInstance, final String json
    ) throws InvalidProtocolBufferException {
    ...
    public static <T extends MessageLite> T parse(
        final Class<T> type, final ByteString bytes
    ) throws InvalidProtocolBufferException {
        return parse(resolveInstance(type), bytes);
    }
    
    public static <T extends MessageLite> T parse(
        final Class<T> type, final ByteString bytes, final boolean json
    ) throws InvalidProtocolBufferException {
        return !json ? parse(resolveInstance(type), bytes) : parse(resolveInstance(type), bytes.toStringUtf8());
    }
    
    public static <T extends MessageLite> T parse(
        final Class<T> type, final String json
    ) throws InvalidProtocolBufferException {
    ...
    private static <T extends MessageLite> T resolveInstance(final Class<T> type) {
        ...
        T instance = LockUtil.exec(INSTANCES_LOCK.readLock(), () -> (T) INSTANCES_MAP.get(type));
        if (instance == null) {
            instance = LockUtil.exec(
                INSTANCES_LOCK.writeLock(),
                () ->
                    (T)
