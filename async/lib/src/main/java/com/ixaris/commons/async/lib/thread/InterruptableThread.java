...
/**
 * Allows a thread to do some uninterruptable work and defer the interrupt till after that work is done. Particularly useful when some code does
 * not correctly handle interrupts.
 *
 * <p>WARNING: the uninterruptable work should NOT block! The intent is to avoid interrupting this work but not to ignore the interrupt
 * indefinitely, otherwise an application might hang.
 *
 * @author brian.vella
 */
    ...
    public static boolean interrupted() {
        ...
        if (interrupted) {
            ...
            if (!ct.state.compareAndSet(INTERRUPTED, INTERRUPTABLE)) {
                throw new IllegalStateException("Thread was interrupted; Expecting internal state to be INTERRUPTED (2) but was " + ct.state.get());
            }
        } else {
