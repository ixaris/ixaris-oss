...
public class CryptoUtil {
            ...
            if (cipher.getAlgorithm().contains("/GCM/")) {
                ...
                if (cipher.getIV() != null) {
                    throw new IllegalStateException(String.format(
                        "Expecting algorithm %s to not generate an IV", cipher.getAlgorithm()
                    ));
                }
                return null;
            }
     ...
     */
    public static int initCipherForDecryptionAndGetOffset(
        final Cipher cipher, final SecretKeySpec key, final byte[] data
    ) {
        try {
            // initialise using the key
            if (cipher.getAlgorithm().contains("/GCM/")) {
    ...
    public static byte[] decrypt(
        final byte[] data, final byte[] key, final String algorithm, final String modeAndPadding
    ) {
        if (data == null) {
            return new byte[] {};
        }
    ...
    public static String decrypt(
        final String data, final byte[] key, final String algorithm, final String modeAndPadding
    ) {
