...
public class ProtocPluginAssembler {
    ...
    private File buildWindowsPlugin(final ProtocPlugin plugin) {
        ...
        try (final PrintWriter out = new PrintWriter(new FileWriter(winRun4JIniFile))) {
            ...
            throw new IllegalStateException(
                "Could not write WinRun4J ini file: " + winRun4JIniFile.getAbsolutePath() + " " + e.getMessage(), e
            );
        }
        ...
        try {
            ...
            throw new IllegalStateException(
                String.format(
                    "Could not copy WinRun4J executable to: %s %s",
                    pluginExecutableFile.getAbsolutePath(), e.getMessage()
                ),
                e
            );
        }
    ...
    private File buildUnixPlugin(final ProtocPlugin plugin) {
        ...
        try (final PrintWriter out = new PrintWriter(new FileWriter(pluginExecutableFile))) {
            ...
            out.println();
            out.println(
                "\"" + new File(javaHome, "bin/java").getAbsolutePath() + "\" -cp $CP " + plugin.getClass().getName()
            );
            out.println();
        } catch (final IOException e) {
    ...
    private boolean archDirectoryExists(final String arch) {
        return javaHome != null
            && (new File(javaHome, "jre/lib/" + arch).isDirectory() || new File(javaHome, "lib/" + arch).isDirectory());
    }
