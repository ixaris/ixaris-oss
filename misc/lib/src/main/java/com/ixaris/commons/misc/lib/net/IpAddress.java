...
public class IpAddress implements Comparable<IpAddress> {
    ...
    private String toStringV6() {
        ...
        for (int i = 0; i < 16; i += 2) {
            if ((address[i] == 0) && (address[i + 1] == 0)) {
                ...
                p[i >> 1] =
                    LEADING_ZEROS.matcher(HexUtil.encode(address, i, i + 1, true)).replaceAll("$1").toLowerCase();
                inZeros = false;
            }
        }
    ...
    public int hashCode() {
        ...
        if (address.length > 4) {
            hash += (address[4] << 24) + (address[5] << 16) + (address[6] << 8) + address[7]
                + (address[8] << 24) + (address[9] << 16) + (address[10] << 8) + address[11]
                + (address[12] << 24) + (address[13] << 16) + (address[14] << 8) + address[15];
        }
