...
public final class DoneCompletionStageAssert<T> extends ObjectAssert<CompletionStage<T>> {
    ...
    protected DoneCompletionStageAssert(final CompletionStage<T> actual) {
        super(actual);
        try {
            result = actual.toCompletableFuture().get();
    ...
    protected DoneCompletionStageAssert(final CompletionStage<T> actual, final long timeoutMs) {
        super(actual);
        try {
            result = actual.toCompletableFuture().get(timeoutMs, TimeUnit.MILLISECONDS);
    ...
    /**
     * Assert that the promise has been fulfilled (i.e. completed successfully with a result) and convert this result
     * using the provided converter function.
     */
    public <U> U isFulfilled(final Function<T, U> resultConverter) {
        if (timedOut || throwable != null) {
            failWithMessage(
                "Expected promise [%s] to be fulfilled - instead it was [%s]", actual, getFormattedOutcomeMessage()
            );
        }
    ...
    /**
     * Assert that the promise has been completed with an exception and convert the failure using the provided converter
     * function.
     */
    public <U> U isRejected(final Function<Throwable, U> failureConverter) {
        if (timedOut || throwable == null) {
            failWithMessage(
                "Expected promise [%s] to be rejected - instead it was [%s]", actual, getFormattedOutcomeMessage()
            );
        }
    ...
    @SuppressWarnings("unchecked")
    public <E extends Throwable, U> U isRejectedWith(
        final Class<E> exceptionType, final Function<E, U> assertFunction
    ) {
    ...
    public DoneCompletionStageAssert<T> isTimedOut() {
        if (!timedOut) {
            failWithMessage(
                "Expected promise [%s] to be timed out, instead it was [%s]", actual, getFormattedOutcomeMessage()
            );
        }
        return this;
    }
    ...
    private String getFormattedOutcomeMessage() {
        if (timedOut) {
            ...
            return Optional.ofNullable(getFormattedThrowableMessage(throwable)).orElse(String.format(
                "Rejected with exception [%s]%n%s", throwable.getMessage(), ExceptionUtil.getStackTrace(throwable)
            ));
        }
    }
    ...
    public static final class ExceptionAssert<E extends Throwable>
    extends AbstractThrowableAssert<ExceptionAssert<E>, E> {
