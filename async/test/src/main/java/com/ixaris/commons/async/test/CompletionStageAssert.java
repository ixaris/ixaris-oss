...
/**
 * Assertions for the {@link CompletionStage}. This class <strong>will block eagerly</strong> until a result/error/timeout happens, and once this
 * is done, it will assert on the result of this wait. This means that if the promise is fulfilled after the timeout, this assertion class
 * <strong>will NOT</strong> reflect that.
 *
 * <p>Example usage:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * If the assertions on the resolved result of the completion stage need to be done using some other custom assertion, a conversion function may
 * be passed to the {@code isFulfilled()} function to allow chaining as follows:
 *
 * <pre>{@code
 * CompletionStageAssert
 *    .assertThat(someCompletionStage)
...
public final class CompletionStageAssert<T> extends ObjectAssert<CompletionStage<T>> {
    ...
    public static <T> CompletionStageAssert<T> assertThat(final CallableThrows<? extends CompletionStage<T>, ?> actual) {
        try {
            return new CompletionStageAssert<>(actual.call());
        } catch (final Throwable t) {
            final CompletableFuture<T> future = new CompletableFuture<>();
            future.completeExceptionally(t);
            return new CompletionStageAssert<>(future);
        }
    }
