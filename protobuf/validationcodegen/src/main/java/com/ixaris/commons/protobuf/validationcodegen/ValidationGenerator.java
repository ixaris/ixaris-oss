...
public final class ValidationGenerator {
    ...
    public static final Pattern FIELD_VALIDATION_PATTERN = Pattern.compile("(?:^| +)(" + REQUIRED + "|" + HAS_TEXT + ")(?=$| +)");
    ...
    static void extractFieldValidationInfoAndDependencies(final String protoPackage,
                                                          final String name,
        ...
        final FieldValidationInfo info = determineFieldTypeInfo(repeated, mapField, javaType, keyJavaTypeSupplier, valueJavaTypeSupplier);
        ...
        switch (info.type) {
            case LIST:
                ...
                if (!keysEnumeration.isEmpty()) {
                    throw new IllegalArgumentException(name + ": keys enumeration only supported for map fields with string keys");
                }
                sb.append("list");
            ...
            default:
                if (!valuesValidation.isEmpty()) {
                    throw new IllegalArgumentException(name + ": values validation only supported for repeated or map fields");
                }
                if (!keysValidation.isEmpty()) {
                    throw new IllegalArgumentException(name + ": keys validation only supported for map fields");
                }
        ...
        if (fieldValidations.containsKey(name)
            ...
            generateSingleFieldValidation(protoPackage,
                name,
                info.type,
                typeName,
                fieldValidation,
    ...
    static void generateSingleFieldValidation(final String protoPackage,
                                              final String name,
                                              final Type type,
                                              final String typeName,
                                              final String fieldValidation,
            ...
            switch (m.group(1)) {
                ...
                case SIZE:
                    if (type != Type.STRING && type != Type.LIST && type != Type.MAP) {
                        throw new IllegalArgumentException(name + ": size applicable to STRING, LIST and MAP, not " + type);
                    }
                ...
                case RANGE:
                    if (type != Type.INT && type != Type.LONG && type != Type.FLOAT && type != Type.DOUBLE) {
                        throw new IllegalArgumentException(name + ": range applicable to INT, LONG, FLOAT and DOUBLE, not " + type);
                    }
                ...
                case IN:
                    ...
                    switch (type) {
                        ...
                        case DOUBLE:
                            sb
                                .append(".in(")
                                .append(inParams.stream().map(p -> new BigDecimal(p).toString()).collect(Collectors.joining(", ")))
                                .append(")");
                            break;
                        case STRING:
                            sb
                                .append(".in(")
                                .append(inParams.stream()
                        ...
                        case ENUM:
                            ...
                            sb
                                .append(".in(")
                                .append(inParams.stream().map(p -> enumType + "." + p).collect(Collectors.joining(", ")))
                                .append(")");
                            break;
                        default:
                            throw new IllegalArgumentException(name + ": in applicable to INT, LONG, FLOAT, DOUBLE, STRING and ENUM, not " + type);
                    }
                    break;
                case NOT_IN:
                    ...
                    switch (type) {
                        ...
                        case DOUBLE:
                            sb
                                .append(".notIn(")
                                .append(notInParams.stream().map(p -> new BigDecimal(p).toString()).collect(Collectors.joining(", ")))
                                .append(")");
                            break;
                        case STRING:
                        ...
                        case ENUM:
                            ...
                            sb
                                .append(".notIn(")
                                .append(notInParams.stream().map(p -> enumType + "." + p).collect(Collectors.joining(", ")))
                                .append(")");
                            break;
                        default:
                            throw new IllegalArgumentException(name
                                + ": not_in applicable to INT, LONG, FLOAT, DOUBLE, STRING and ENUM, not "
                                + type);
                    }
                    break;
                case REGEX:
    ...
    private static FieldValidationInfo determineFieldTypeInfo(final boolean repeated,
                                                              final boolean mapField,
                                                              final JavaType javaType,
                                                              final Supplier<JavaType> keyJavaTypeSupplier,
                                                              final Supplier<JavaType> valueJavaTypeSupplier) {
    ...
    static void generateSingleMessageValidation(final String messageValidation,
                                                final StringBuilder sb,
                                                final Map<String, FieldValidationInfo> fieldValidations) {
    ...
    private static void createAndAddMessageValidationInfo(final String valMethod,
    ...
    private static String resolveJavaEnum(final String type, final String protoPackage, final Map<String, String> fqnProtoEnumToJava) {
