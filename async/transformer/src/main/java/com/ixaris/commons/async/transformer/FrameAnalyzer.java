...
final class FrameAnalyzer extends Analyzer<BasicValue> {
     ...
     */
    private static final BasicInterpreter TYPE_INTERPRETER = new BasicInterpreter(ASM8) {
        ...
        public BasicValue binaryOperation(final AbstractInsnNode insn, final BasicValue value1, final BasicValue value2) throws AnalyzerException {
            if (insn.getOpcode() == AALOAD) {
                // preserve the array element type; value1 os the array type, i.e. the element type prepended by [
                // e.g. [Ljava/lang/String for string, value2 is the index. Thus stripping the first character gives
                // us the element type
                return new ExtendedValue(Type.getType(value1.getType().getInternalName().substring(1)));
            } else {
