...
public class MinCascadeValueValidation<T extends Value> extends CascadeValueValidation<T> {
    ...
    public <V extends T> void validate(final V value, final V nextMatchingValue) throws ConfigValidationException {
        ...
        if (!(value instanceof Comparable<?>)) {
            throw new IllegalArgumentException("MinMaxCascadeValueValidation is only applicable to Comparable values. Value given " + value);
        }
        ...
        if (nextMatchingValue == null) {
            ...
            throw new ConfigValidationException("Value [" + value + "] should be larger than [" + nextMatchingValue + "]");
        }
    }
