...
public final class SetUtils {
    ...
    /**
     * Given a set s and an integer k, this method generates all <strong>combinations</strong> of size k from s. Note
     * that this generates combinations, not permutations (i.e. {a,b} = {b,a}). This method works in terms of sets, thus
     * there will be no duplicates and no ordering on the resulting set.
     *
     * <p>This method works by taking the first element out of the set, and then generating all the combinations of k-1
     * from the resulting subset. Then, we add the first element to all of the generated combinations. Next, we generate
     * all the combinations of size k from the resulting subset. Thus, there are two recursive calls. The base cases
     * are:
     *
     * <ul>
